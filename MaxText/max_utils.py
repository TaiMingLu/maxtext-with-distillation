...
def kl_divergence_between_logits(
    ...
    *,
    ...
    truncation_strategy: str = "renormalize",
) -> jnp.ndarray:
  ...
  Args:
    ...
    top_k: If set, keep only the highest-probability ``k`` tokens in the teacher
      distribution before computing the KL term.
    top_p: If set, keep the smallest prefix of tokens whose cumulative teacher
      probability mass is at least ``p``.
    truncation_strategy: "renormalize" drops gradients outside the kept set by
      renormalizing the teacher over the surviving tokens. "other_bucket"
      aggregates the discarded probability mass into a single additional
      category so student probability outside the kept set is still penalized.
  ...
  if top_k in (0, None) and (top_p in (0.0, None)):
    # KL(teacher || student) = sum p_t * (log p_t - log p_s)
    return jnp.sum(teacher_probs * (teacher_log_probs - student_log_probs), axis=-1)

  if top_k not in (None, 0) and top_p not in (None, 0.0):
    raise ValueError("Only one of top_k or top_p may be specified for KD truncation.")
  ...
  if top_k not in (None, 0):
    k = int(min(top_k, vocab_size))
    if k >= vocab_size:
      mask = jnp.ones_like(teacher_probs, dtype=bool)
    else:
      # jax.lax.top_k requires k > 0.
      k = max(k, 1)
      _, top_indices = jax.lax.top_k(teacher_probs, k)
      mask = jnp.sum(
          jax.nn.one_hot(top_indices, vocab_size, dtype=teacher_probs.dtype), axis=-2
      ) > 0
  else:
    p = float(top_p) if top_p not in (None, 0.0) else 1.0
    p = max(min(p, 1.0), eps)
    sorted_probs, sorted_indices = jax.lax.top_k(teacher_probs, vocab_size)
    cumulative = jnp.cumsum(sorted_probs, axis=-1)
    # Count tokens strictly below the threshold and include the first token
    # that pushes the cumulative mass over p.
    num_below = jnp.sum(cumulative < p, axis=-1)
    num_to_keep = jnp.clip(num_below + 1, 1, vocab_size)
    arange = jnp.arange(vocab_size, dtype=jnp.int32)
    keep_sorted = arange < num_to_keep[..., None]
    mask = jnp.sum(
        jax.nn.one_hot(sorted_indices, vocab_size, dtype=teacher_probs.dtype)
        * keep_sorted[..., None],
        axis=-2,
    ) > 0

  kept_teacher_probs = jnp.where(mask, teacher_probs, 0.0)
  teacher_mass = jnp.sum(kept_teacher_probs, axis=-1, keepdims=True)

  if truncation_strategy == "renormalize":
    renorm_teacher_probs = kept_teacher_probs / (teacher_mass + eps)
    renorm_teacher_log_probs = jnp.log(renorm_teacher_probs + eps)
    return jnp.sum(
        renorm_teacher_probs * (renorm_teacher_log_probs - student_log_probs), axis=-1
    )

  if truncation_strategy == "other_bucket":
    teacher_other = jnp.clip(1.0 - teacher_mass[..., 0], a_min=0.0)
    student_probs = jnp.exp(student_log_probs)
    student_kept = jnp.where(mask, student_probs, 0.0)
    student_mass = jnp.sum(student_kept, axis=-1)
    student_other = jnp.clip(1.0 - student_mass, a_min=eps)

    kept_teacher_log_probs = jnp.log(kept_teacher_probs + eps)
    kl_kept = jnp.sum(
        kept_teacher_probs * (kept_teacher_log_probs - student_log_probs), axis=-1
    )
    kl_other = teacher_other * (
        jnp.log(teacher_other + eps) - jnp.log(student_other + eps)
    )
    return kl_kept + kl_other

  raise ValueError(
      "truncation_strategy must be 'renormalize' or 'other_bucket', got "
      f"{truncation_strategy!r}."
  )
