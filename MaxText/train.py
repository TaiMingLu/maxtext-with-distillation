...
def _split_kd_state(state):
  teacher_params = state.params["teacher_params"]
  new_state = state.replace(params={k: v for k, v in state.params.items() if k != "teacher_params"})
  return new_state, teacher_params  


def _merge_kd_state(state, teacher_params):
  return state.replace(params=dict(state.params, teacher_params=teacher_params))

# Global handle for an optional separate teacher model when KD is enabled.
TEACHER_MODEL = None
...
def kd_loss_fn(model, config, data, dropout_rng, params, teacher_params, is_train=True):
  """Standard CE + KL distillation loss.

  teacher_params are treated as frozen parameters.
  """
  rng1, aqt_rng = jax.random.split(dropout_rng)
  ...
  # Teacher forward pass (no dropout, immutable). Use separate teacher model if available.
  ...
  kd_truncation_strategy = getattr(config, "kd_truncation_strategy", "renormalize")
  kd_kl = max_utils.kl_divergence_between_logits(
      logits,
      teacher_logits,
      kd_temperature,
      top_k=kd_top_k or None,
      top_p=kd_top_p or None,
      truncation_strategy=kd_truncation_strategy,
  )
  ...
  aux = {
      "intermediate_outputs": intermediate_outputs,
      "total_loss": total_ce,  # keep raw CE total for consistency with aggregation
      "total_weights": total_weights,
      "moe_lb_loss": moe_lb_loss,
      "mtp_loss": mtp_loss,
      "kd_loss": kd_loss,
...
def loss_fn(model, config, data, dropout_rng, params, is_train=True):
  ...
  aux = {
      ...
      "moe_lb_loss": moe_lb_loss,
      "mtp_loss": mtp_loss,
      "ce_loss": (total_loss / (total_weights + EPS)),
  }
  return loss, aux
...
def train_step(model, config, state_mesh_shardings, state, data, dropout_rng, sparse=False):
  ...
  elif getattr(config, "use_kd", False):
    state, teacher_params = _split_kd_state(state)
    state_mesh_shardings, teacher_params_sharding = _split_kd_state(state_mesh_shardings)
    extra_dpo_args = [teacher_params]
    _loss_fn = kd_loss_fn
  ...
  if config.gradient_accumulation_steps > 1:
    ...
    def accumulate_gradient(acc_grad_and_loss, data):
      ...
      # Accumulate raw CE total (token-summed) for correct averaging later
      acc_grad_and_loss["loss"] += aux["total_loss"]
      acc_grad_and_loss["moe_lb_loss"] += aux["moe_lb_loss"]
      acc_grad_and_loss["mtp_loss"] += aux["mtp_loss"]
      if getattr(config, "use_kd", False):
        # Track token-weighted KD so we can average correctly across microbatches
        acc_grad_and_loss["kd_weighted_sum"] += aux.get("kd_loss", 0.0) * aux["total_weights"]
    ...
    if getattr(config, "use_kd", False):
      init_grad_and_loss["kd_weighted_sum"] = 0.0

    grad_and_loss, aux = jax.lax.scan(
        accumulate_gradient, init_grad_and_loss, data, length=config.gradient_accumulation_steps
    )
    # Compute token-averaged CE and KD for logging consistency
    ce_avg = grad_and_loss["loss"] / (grad_and_loss["total_weights"] + EPS)
    kd_avg = 0.0
    kd_alpha = getattr(config, "kd_alpha", 0.5)
    if getattr(config, "use_kd", False):
      kd_avg = grad_and_loss["kd_weighted_sum"] / (grad_and_loss["total_weights"] + EPS)
    # Reconstruct reported loss using alpha weighting (matches kd_loss_fn)
    loss = (
        (1.0 - kd_alpha) * ce_avg
        + (kd_alpha * kd_avg if getattr(config, "use_kd", False) else 0.0)
    ...
    # Ensure CE/KD metrics reflect proper averages even under accumulation
    aux_ce_loss_for_log = ce_avg
    aux_kd_loss_for_log = kd_avg if getattr(config, "use_kd", False) else 0.0
  else:
    if config.optimizer_memory_host_offload:
      if getattr(config, "use_dpo", False):
        ...
        extra_dpo_args = [reference_params]
      if getattr(config, "use_kd", False):
        teacher_params = jax.device_put(teacher_params, max_utils.with_memory_kind(teacher_params_sharding, "device"))
        extra_dpo_args = [teacher_params]
    grad_func = jax.value_and_grad(_loss_fn, argnums=4, has_aux=True)
  ...
  # Report CE/KD with correct normalization under both GA and non-GA paths
  if config.gradient_accumulation_steps > 1:
    if getattr(config, "use_kd", False):
  ...
  if getattr(config, "use_kd", False):
    new_state = _merge_kd_state(new_state, teacher_params)
...
def eval_step(model, config, state, data, dropout_rng):
  ...
  metrics = {
      "scalar": {
          ...
          "evaluation/ce_loss": aux.get("ce_loss", total_loss / (total_weights + EPS)),
      },
  }
  if getattr(config, "use_dpo", False):
    metrics["scalar"]["evaluation/dpo_reward_accuracy"] = aux["reward_accuracy"]
  if getattr(config, "use_kd", False):
    metrics["scalar"]["evaluation/kd_loss"] = aux.get("kd_loss", 0.0)
...
def setup_train_loop(config, recorder, devices=None):
  ...
  with maybe_record_goodput(recorder, GoodputEvent.TRAINING_PREPARATION):
    ...
    if getattr(config, "use_kd", False):
      ...
      try:
        tmp = checkpointing.load_params_from_path(
            config.kd_teacher_parameters_path,
            teacher_abstract_state.params,
            config.checkpoint_storage_concurrent_gb,
            use_ocdbt=config.checkpoint_storage_use_ocdbt,
            use_zarr3=config.checkpoint_storage_use_zarr3,
        )
        teacher_params = tmp["params"] if isinstance(tmp, dict) and "params" in tmp else tmp
      except Exception:
        # 2) Fallback to full TrainState restore from a step directory
        step_dir = epath.Path(config.kd_teacher_parameters_path)
        if step_dir.name == "items":
          step_dir = step_dir.parent
        try:
          restored_state = ocp.StandardCheckpointer().restore(step_dir, teacher_abstract_state)
          teacher_params = restored_state.params["params"]
      ...
      state_mesh_shardings = _merge_kd_state(state_mesh_shardings, teacher_params_sharding)
...
def train_loop(config, recorder, state=None):
  ...
  if getattr(config, "use_kd", False):
    assert "teacher_params" in state.params, "KD is enabled but teacher_params missing. Ensure teacher checkpoint loads during setup."
    # teacher sharding was already populated in setup_train_loop; no overwrite here
