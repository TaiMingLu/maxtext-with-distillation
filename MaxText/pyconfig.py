...
def validate_keys(keys):
  ...
  if keys.get("use_kd", False):
    ...
    kd_top_k = keys.get("kd_top_k", 0) or 0
    kd_top_p = keys.get("kd_top_p", 0.0) or 0.0
    if kd_top_k and kd_top_p:
      raise ValueError("Only one of kd_top_k or kd_top_p may be set when use_kd=True.")
    if kd_top_k < 0:
      raise ValueError("kd_top_k must be non-negative.")
    if not 0.0 <= kd_top_p <= 1.0:
      raise ValueError("kd_top_p must lie in [0, 1].")
    kd_truncation_strategy = keys.get("kd_truncation_strategy", "renormalize")
    if kd_truncation_strategy not in ("renormalize", "other_bucket"):
      raise ValueError(
          "kd_truncation_strategy must be either 'renormalize' or 'other_bucket'."
      )
